...
SeqCanvasUI::SeqCanvasUI(Ren::Context &ctx, const Gui::BitmapFont &font,
    ...
    : Gui::BaseElement(pos, size, parent),
      ...
      time_cursor_{ctx,
                   ...
                   Ren::Vec2f{2.0f, 2.0f},
                   this},
      element_normal_(ctx, "assets_pc/textures/editor/seq_el.uncompressed.tga",
                      Ren::Vec2f{12.5f, 1.5f}, 1.0f, Ren::Vec2f{-1.0f, -1.0f},
                      Ren::Vec2f{2.0f, 2.0f}, this),
      element_highlighted_(
          ctx, "assets_pc/textures/editor/seq_el_highlighted.uncompressed.tga",
          Ren::Vec2f{12.5f, 1.5f}, 1.0f, Ren::Vec2f{-1.0f, -1.0f}, Ren::Vec2f{2.0f, 2.0f},
          this),
...
void SeqCanvasUI::Draw(Gui::Renderer *r) {
        ...
        if (sequence_) {
            ...
            for (int action = 0; action < actions_count; action++) {
                ...
                if (seq_action->sound_wave_tex) {
                    ...
                    const float x_beg_sound = GetPointFromTime(
                ...
                float y_text_pos =
                    y_coord + track_height - 2.0f * border_height - font_height;

                font_.DrawText(r, str_buf,
                               SnapToPixels(Ren::Vec2f{x_beg + 1.25f * crop_region_width,
                                                       y_text_pos}),
                               Gui::ColorCyan, this);
                ...
                if (seq_action->anim_ref) {
                    ...
                    font_.DrawText(r, anim_name,
                                   SnapToPixels(Ren::Vec2f{
                                       x_beg + 1.25f * crop_region_width, y_text_pos}),
                                   Gui::ColorWhite, this);
                }
    ...
    if (time_cur_ >= time_range_[0] && time_cur_ <= time_range_[1]) {
        ...
        const Ren::Vec2f time_pos = SnapToPixels(Ren::Vec2f{
            -1.0f +
                2.0f * (time_cur_ - time_range_[0]) / (time_range_[1] - time_range_[0]) -
                10.0f / dims_px_[1][0],
            -1.0f});
...
void SeqCanvasUI::Hover(const Ren::Vec2f &p) {
    ...
    if (selected_index_ != Ren::Vec2i{-1, -1}) {
        ...
        if (action) {
            ...
            const float time_delta_discrete =
                std::floor(time_delta / ElementMoveStep) * ElementMoveStep;
            ...
            const double duration =
                std::max(action->time_end - action->time_beg, (double)ElementDurationMin);
...
SeqAction *SeqCanvasUI::GetActionAtPoint(const Ren::Vec2f &p, Ren::Vec2i &out_index,
                                         uint32_t &flags) {
    using namespace SeqCanvasUIInternal;
        ...
        for (int track = 0; track < TrackCount; track++) {
            ...
            for (int action = 0; action < actions_count; action++) {
                ...
                if (p[0] > x_beg && p[0] < x_end && p[1] > y_coord + border_height &&
                    ...
                    const float CropRegionWidth =
                        float(2 * ElementCropRegionPx) / float(size_px()[0]);
