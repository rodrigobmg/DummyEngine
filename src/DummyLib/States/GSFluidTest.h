#pragma once

#include <condition_variable>
#include <mutex>
#include <thread>

#include <Eng/GameBase.h>
#include <Eng/GameState.h>
#include <Eng/Gui/BaseElement.h>
#include <Eng/Gui/BitmapFont.h>
#include <Eng/Renderer/FrameBuf.h>
#include <Ren/Camera.h>
#include <Ren/MVec.h>
#include <Ren/Mesh.h>
#include <Ren/Program.h>
#include <Ren/SW/SW.h>
#include <Ren/Texture.h>
#include <Sys/Optional.h>

#include "GSBaseState.h"

class Cmdline;
class DebugInfoUI;
class GameStateManager;
class FontStorage;
class SceneManager;
class PagedReader;

class GSFluidTest : public GSBaseState {
    uint64_t last_frame_time_ = 0;
    double cur_fps_ = 0.0;

    uint32_t click_time_ = 0;

    std::shared_ptr<Gui::BitmapFont> book_main_font_, book_emph_font_, book_caption_font_;
    float test_time_counter_s = 0.0f;

    Ren::Vec3f view_origin_, view_dir_;
    float view_fov_, view_offset_ = 0.0f;
    float max_exposure_ = 1000.0f;

    uint32_t plane_index_ = 0xffffffff;

    struct FlCell {
        float velocity[3];
        float density;
    };
    
    std::unique_ptr<FlCell[]> fl_grid_;

    void OnPostloadScene(JsObject &js_scene) override;
    void OnUpdateScene() override;

    void Draw(uint64_t dt_us) override;
    void DrawUI(Gui::Renderer *r, Gui::BaseElement *root) override;

    void UpdatePlaneMaterial(Ren::Material *plane_mat, float delta_time_s);

  public:
    explicit GSFluidTest(GameBase *game);
    ~GSFluidTest() final = default;

    void Enter() override;
    void Exit() override;

    bool HandleInput(const InputManager::Event &evt) override;
};