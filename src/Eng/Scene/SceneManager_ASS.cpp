...
uint32_t Crc32HashFile(const char *in_file) {
...
bool CheckCanSkipAsset(const char *in_file, const char *out_file, assets_context_t &ctx) {
    ...
    if (in_ndx) {
        ...
        ctx.log->Info("[PrepareAssets] Hashing %s", in_file);
        const uint32_t in_crc32_hash = Crc32HashFile(in_file);
        ...
        if (js_in_file.Has("in_hash") && js_in_file.Has("out_hash")) {
            ...
            if (js_in_file_hash.val == in_crc32_hash_str) {
                ctx.log->Info("[PrepareAssets] Hashing %s", out_file);
                const uint32_t out_crc32_hash = Crc32HashFile(out_file);
...
bool SceneManager::PrepareAssets(const char *in_folder, const char *out_folder,
    ...
    auto convert_file = [out_folder, &last_db_write](assets_context_t &ctx,
        ...
        if (res) {
            ctx.log->Info("[PrepareAssets] Hashing %s", out_file.c_str());
            const uint32_t out_crc32_hash = Crc32HashFile(out_file.c_str());
            const std::string out_crc32_hash_str = std::to_string(out_crc32_hash);
